...
public class AddUserActivity extends AppCompatActivity {
    ...
    private RadioGroup roleRdGrp;
    private RadioButton teacherRdBtn;
    private RadioButton adminRdBtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addUserTxt = (TextView)findViewById(R.id.add_user_txtview);
        roleTxt = (TextView)findViewById(R.id.role_txtview);
        roleRdGrp = (RadioGroup)findViewById(R.id.role_rdGrp);
        teacherRdBtn = (RadioButton)findViewById(R.id.teacher_rdBtn);
        adminRdBtn = (RadioButton)findViewById(R.id.admin_rdBtn);

        myIntent = getIntent();
        final String role = myIntent.getStringExtra("role");
        if(role.equals("Teacher")) {
            addUserTxt.setText("Add new student");
            roleRdGrp.setVisibility(View.GONE);
            roleTxt.setVisibility(View.GONE);
            phone.setHint("Guardian's Contact Number");
        }
                                        ...
                                        .addOnCompleteListener(AddUserActivity.this, new OnCompleteListener<AuthResult>() {
                                            ...
                                            public void onComplete(@NonNull Task<AuthResult> task) {
                                                if (task.isSuccessful()) {
                                                    ...
                                                    String roleParam = "Student";
                                                    if(role.equals("Admin")){
                                                        if(adminRdBtn.isChecked())
                                                            roleParam = "Admin";
                                                        else
                                                            roleParam = "Teacher";
                                                        currentTeach[0] = null;
                                                    ...
                                                    Toast.makeText(AddUserActivity.this, "Adding Failed.", Toast.LENGTH_SHORT).show();
                                                }

                                            }
                                        });
                            } else
                                Toast.makeText(AddUserActivity.this, "Minimum password length is 6.", Toast.LENGTH_LONG).show();
                        } else
                            Toast.makeText(AddUserActivity.this, "Minimum phone length is 8.", Toast.LENGTH_LONG).show();
                    } else
                        Toast.makeText(AddUserActivity.this, "Password does not match.", Toast.LENGTH_LONG).show();
                }
                else
                    Toast.makeText(AddUserActivity.this, "Please complete the fields.", Toast.LENGTH_LONG).show();
            }
        });
